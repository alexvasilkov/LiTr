        ...
        if (selectedTrack == sourceTrack || selectedTrack == NO_SELECTED_TRACK) {
            ...
            if (tag >= 0) {
                ...
                if (bytesRead < 0 || (sampleFlags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                    ...
                    decoder.queueInputFrame(frame);
                    mediaSource.advance();
                    // LogUtils.d(TAG, "Sample time: " + sampleTime + ", source bytes read: " + bytesRead);
                }
            } else {
                switch (tag) {
                    case MediaCodec.INFO_TRY_AGAIN_LATER:
                        // LogUtils.d(TAG, "Will try getting decoder input buffer later");
                        break;
                    default:
                        LogUtils.e(TAG, "Unhandled value " + tag + " when decoding an input frame");
                        break;
                }
            }
        }
    ...
    private int queueDecodedInputFrame() throws TrackTranscoderException {
        ...
        if (tag >= 0) {
            ...
            if ((decoderOutputFrame.bufferInfo.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                LogUtils.d(TAG, "EoS on decoder output stream");
            ...
            switch (tag) {
                ...
                case MediaCodec.INFO_OUTPUT_FORMAT_CHANGED:
                    ...
                    LogUtils.d(TAG, "Decoder output format changed: " + sourceAudioFormat);
                    break;
                default:
    ...
    private int writeEncodedOutputFrame() throws TrackTranscoderException {
        ...
        if (tag >= 0) {
            ...
            if ((frame.bufferInfo.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                LogUtils.d(TAG, "Encoder produced EoS, we are done");
                progress = 1.0f;
            ...
            switch (tag) {
                case MediaCodec.INFO_TRY_AGAIN_LATER:
                    // LogUtils.d(TAG, "Will try getting encoder output buffer later");
                    break;
                case MediaCodec.INFO_OUTPUT_FORMAT_CHANGED:
                    ...
                    LogUtils.d(TAG, "Encoder output format received " + outputMediaFormat);
                    break;
                default:
                    LogUtils.e(TAG, "Unhandled value " + tag + " when receiving encoded output frame");
                    break;
            }
        }
